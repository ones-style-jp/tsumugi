...
create policy "staff_view_own_store" on public.stores
  for select using (id = public.current_user_store_id() or public.is_admin());

-- 管理者のみ更新可能
create policy "admin_update_store" on public.stores
  for update using (public.is_admin() and id = public.current_user_store_id());
...
create policy "staff_view_same_store" on public.staff
  for select using (store_id = public.current_user_store_id() or public.is_admin());

-- 管理者のみ作成・更新・削除可能
create policy "admin_manage_staff" on public.staff
  for all using (public.is_admin() and store_id = public.current_user_store_id())
...
create policy "staff_manage_cm_offices" on public.cm_offices
  for all using (public.is_staff() and store_id = public.current_user_store_id())
  with check (store_id = public.current_user_store_id());
...
-- =============================================
-- パターン: スタッフは自店全部、家族は自分の患者のみ (家族はモニタリング・体力測定・連絡帳は閲覧)
-- 日誌は社内向けなので家族閲覧不可
...
-- 13. audit_logs ポリシー
-- =============================================
-- 管理者のみ閲覧、書き込みは関数経由 (INSERT は全員可能)
create policy "admin_view_audit" on public.audit_logs
  for select using (public.is_admin() and store_id = public.current_user_store_id());
